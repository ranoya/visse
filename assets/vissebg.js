...
function draw() {
  ...
  if (vai[1] > 0.7) {
    // 1
    curva(20 + mx / 5, 20, 400, 430, 0.02, false);
  }

  if (vai[2] > 0.7) {
    // 2
    curva(400 + mx / 2, 450, 300, 380, 0.04, false);
  }

  if (vai[3] > 0.7) {
    // 3
    curva(700 + mx / 8, 1200, 700, 810, 0.01, false);
  }

  if (vai[4] > 0.7) {
    // 4
    curva(1200 + mx / 1.5, -800, 1000, 1070, 0.006, false);
  }

  if (vai[5] > 0.7) {
    // 5
    curva(2000 + mx / 6, 350, 1340, 1470, 0.003, false);
  }

  if (vai[6] > 0.7) {
    // 6
    curva(-10 + mx / 3, 820, 540, 590, 0.006, false);
  }

  if (vai[7] > 0.7) {
    // 8
    curva(1200 + mx / 8, 650, 280, 320, 0.04, false);
  }

  if (vai[8] > 0.7) {
    // 7
    curva(2850 + mx * 2, 1250, 2040, 2070, 0.002, false);
  }

  if (vai[9] > 0.7) {
    // 9
    curva(-480 + mx / 15, 1220, 880, 910, 0.003, false);
  }

  if (vai[10] > 0.7) {
    // 10
    curva(-60 + mx * 1.4, -120, 200, 300, 0.01, false);
  }

  if (vai[11] > 0.7) {
    // 11
    curva(2300 + mx / 10, -450, 1200, 1350, 0.003, false);
  }

  if (vai[12] > 0.7) {
    // 12
    curva(850 + mx / 4, 320, 100, 140, 0.03, false);
  }
